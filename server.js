...
// - Seed dev admins: node server.js --seed [--seed-reset-passwords]
// ============================================================
...
// 0) CLI FLAGS
// ============================================================
const IS_SEED = process.argv.includes("--seed");
const SEED_RESET_PASSWORDS = process.argv.includes("--seed-reset-passwords");
...
// Optional override for overall admin seed password
const OVERALL_ADMIN_PASSWORD = String(process.env.OVERALL_ADMIN_PASSWORD || "ovadmin");

// Optional override for branch admin seed password
const BRANCH_ADMIN_PASSWORD = String(process.env.BRANCH_ADMIN_PASSWORD || "admin");
...
usersRouter.put("/:email/profile", authMiddleware, async (req, res) => {
  try {
    ...
    const u = await User.findOneAndUpdate({ email: target }, { $set: update }, { new: true }).lean();
    if (!u) return res.status(404).json({ success: false, error: "User not found" });
...
usersRouter.post("/:email/balances", authMiddleware, async (req, res) => {
  try {
    ...
    const user = await User.findOneAndUpdate({ email }, { $set: setBalances }, { new: true }).lean();
    if (!user) return res.status(404).json({ success: false, error: "User not found" });
...
// ============================================================
// 11) CLIENTS ROUTER (MUST be mounted)
// ============================================================
try {
...
// ============================================================
// 15) SEED ADMINS (node server.js --seed)
// ============================================================
async function seedAdmins({ resetPasswords = false } = {}) {
  console.log("Seeding dev admin accounts...");

  // ✅ Added Mbala + Kitwe here
  const branchEmails = [
  ...
  if (!ov) {
    const passwordHash = await bcrypt.hash(OVERALL_ADMIN_PASSWORD, SALT_ROUNDS);
    await User.create({ email: overall, name: "Overall Admin", role: "ovadmin", passwordHash });
    console.log(`✅ Created overall admin: ${overall} / ${OVERALL_ADMIN_PASSWORD}`);
  } else if (resetPasswords) {
    const passwordHash = await bcrypt.hash(OVERALL_ADMIN_PASSWORD, SALT_ROUNDS);
    await User.updateOne({ email: overall }, { $set: { passwordHash, role: "ovadmin" } });
    console.log(`🔁 Reset overall admin password: ${overall} / ${OVERALL_ADMIN_PASSWORD}`);
  } else {
    console.log(`ℹ️ Overall admin already exists: ${overall}`);
  }
  ...
  for (const e of branchEmails) {
    ...
    if (!u) {
      const passwordHash = await bcrypt.hash(BRANCH_ADMIN_PASSWORD, SALT_ROUNDS);
      await User.create({ email: normalized, name: "Branch Admin", role: "branch_admin", passwordHash });
      console.log(`✅ Created branch admin: ${normalized} / ${BRANCH_ADMIN_PASSWORD}`);
    } else if (resetPasswords) {
      const passwordHash = await bcrypt.hash(BRANCH_ADMIN_PASSWORD, SALT_ROUNDS);
      await User.updateOne({ email: normalized }, { $set: { passwordHash, role: "branch_admin" } });
      console.log(`🔁 Reset branch admin password: ${normalized} / ${BRANCH_ADMIN_PASSWORD}`);
    } else {
      console.log(`ℹ️ Branch admin already exists: ${normalized}`);
    }
  }
...
async function start() {
  try {
    ...
    if (IS_SEED) {
      await seedAdmins({ resetPasswords: SEED_RESET_PASSWORDS });
      await mongoose.disconnect();
      console.log("MongoDB disconnected. Seed complete. Exiting.");
      process.exit(0);
      return;
    }
